...
from tkinter import filedialog, Text, Listbox, Toplevel, messagebox
import threading
import requests
import schedule
import time
...
# Global dictionary to keep track of the checkboxes and their state
checkboxes = {}
...
def launch_gui():
    ...
    def on_send_message():
        global checkboxes
        selected_contacts = [contact_id for contact_id, var in checkboxes.items() if var.get()]
        print(f"Selected contacts: {selected_contacts}")  # Should show selected contact IDs
        for contact_id, var in checkboxes.items():
            print(f"Checkbox for {contact_id}: {var.get()}")  # Debug print to check checkbox states
        message = custom_message_entry.get("1.0", tk.END).strip()
        print(f"Message: {message}")  # Should show the message
        if selected_contacts and message:
            for contact_id in selected_contacts:
                send_message(contact_id, message)
            messagebox.showinfo("Success", "Messages sent successfully.")
        else:
            messagebox.showwarning("Warning", "Please select at least one contact and enter a message.")
    ...
    # Scrollable frame to hold the checkboxes for contacts
    contacts_frame = tk.Frame(root)
    canvas = tk.Canvas(contacts_frame)
    scrollbar = tk.Scrollbar(contacts_frame, orient="vertical", command=canvas.yview)
    ...
    contacts_frame.grid(row=4, column=0, columnspan=2)
    canvas.pack(side="left", fill="both", expand=True)
    scrollbar.pack(side="right", fill="y")

    def variable_trace(*args):
        print(checkboxes[args[0]].get())

    def update_contacts():
        global checkboxes  # Ensuring we use the global variable
        ...
        for contact in contacts:
            var = tk.BooleanVar()
            var.trace("w", lambda *args, var=var: variable_trace(contact['id']))
            cb = tk.Checkbutton(scrollable_frame, text=contact['id'], variable=var)
            cb.pack(anchor='w')
            checkboxes[contact['id']] = var

    custom_message_label = tk.Label(root, text="Custom Message:")
    custom_message_entry = tk.Text(root, height=4, width=50)
    send_message_button = tk.Button(root, text="Send Message", command=on_send_message)

    custom_message_label.grid(row=6, column=0)
    custom_message_entry.grid(row=6, column=1)
    send_message_button.grid(row=7, column=1)

    update_contacts()  # Initial population of contacts
